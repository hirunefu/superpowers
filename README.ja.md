...
- **操作系 VCS は jj（Jujutsu）**：コミット、ブックマーク、マージ、隔離の指示は git と colocate した jj で書かれています。隔離ワークスペースは `jj workspace add` で作り、git worktree はハーネス所有の隔離と jj 非導入時のフォールバックにだけ残しています。
- **経験的に調律されたスキル**：スキルの変更は、白紙の executor サブエージェントに現実的なシナリオを実行させる評価ループで検証し、再評価を通過した修正だけを取り込みます。
- **Claude Code 専用のパッケージング**：他ハーネス向けの統合は上流とのマージでツリーに残りますが、このフォークでは検証していません。必要なら上流を使ってください。
...
`obra/superpowers` の最新の変更をこのフォークに取り込みます（フォークのコミットは公開済みなので、rebase ではなくマージで取り込みます）:

```bash
jj git fetch --remote upstream
jj new main main@upstream    # マージコミット。競合を解決し、検証したら:
...
2. **using-git-worktrees** - 設計承認後に発火。jj workspace による隔離ワークスペースを作り（ブックマークは事前ではなく完了時に命名する）、プロジェクトのセットアップを実行し、クリーンなテストのベースラインを検証する。

3. **writing-plans** - 承認済みの設計とともに発火。作業を一口サイズのタスク（各2〜5分）に分解する。すべてのタスクには正確なファイルパス、完全なコード、検証手順が含まれる。

4. **subagent-driven-development** または **executing-plans** - プランとともに発火。タスクごとに新鮮なサブエージェントを割り当て、タスク単位のレビューと最終のブランチ全体レビューを行う。あるいはこのセッション内で全タスクを最後まで実行する。
...
7. **finishing-a-development-branch** - タスク完了時に発火。テストを検証し、選択肢（マージ / PR / 保持 / 破棄）を提示し、ワークスペースを片付ける。
